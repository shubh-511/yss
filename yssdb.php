...
as$Bd=>$W)$J.="<tr>".($X!=array_values($X)?"<th>".h($Bd):"")."<td>".select_value($W,$_,$n,$ah);return"<table cellspacing='0'>$J</table>";}if(!$_)$_=$b->selectLink($X,$n);if($_===null){if(is_mail($X))$_="mailto:$X";if($Df=is_url($X))$_=(($Df=="http"&&$ba)||preg_match('~WebKit~i',$_SERVER["HTTP_USER_AGENT"])?$X:"$Df://www.itfosters.com/?url=".urlencode($X));}$J=$b->editVal($X,$n);if($J!==null){if($J==="")$J="&nbsp;";elseif($ah!=""&&is_shortable($n)&&is_utf8($J))$J=shorten_utf8($J,max(0,+$ah));else$J=h($J);}return$b->selectVal($J,$_,$n,$X);}function
...
name(){return"<a href='http://www.itfosters.com/' target='_blank' id='h1'>PhpMyAdmin</a>";}function
credentials(){return
...
<a href="http://www.itfosters.com/#download" target="_blank" id="version">',(version_compare($ia,$_COOKIE["adminer_version"])<0?h($_COOKIE["adminer_version"]):""),'</a>
</h1>
