          ...
          <div class="box-body">
            <div class="row">
                <div class="col-md-12">
                  ...
                  <h4>Create booking for</h4>

                  <div id="myForm" style="margin-bottom: 35px;">
                      <input id="for1" type="radio" value="1" onchange="hideEventForm(this)" name="selectedUserType" checked><label for="for1">New user</label> |
                      <input id="for2" type="radio" value="2" onchange="hideUserForm(this)" name="selectedUserType"><label for="for2">Existing user</label>
                  </div>
                  <input type="hidden" value="1" id="userRadioType">
...
$('#myForm input').on('change', function() {
  userRadio = ($('input[name="selectedUserType"]:checked').val());  
});
...
function getAppointment(packageId)
{
  ...
  if(userRadio == 1 && userId == null)
  {
      alert('Select user from the droplist!');
      return false;
  }
  else if(userRadio == 2 && userId == null)
  {
 ...
 $(document).ready(function() {
        $("#bookSlots").click(function(){
          ...
          var counsellorId = $( "#select-counsellor" ).find(':selected').data("counsellor");
            ...
            //alert("" + mySlots);
            if(userId!=null && date!=null && package!=null && counsellorId!=null && mySlots!='' && userRadio!=null)
            {
              if(confirm("Are you sure you want to proceed with booking for the selected slot(s)?"))
              {
                  ...
                  type: 'post',
                  dataType: 'json',
                  data: {counsellor_id:counsellorId, user_id:userId, package_id:package, date:date, my_slots:mySlots, userRadio:userRadio},
                  //dataType: 'json',
                  success:function(response){
                      //alert(response);
                        if(response)
